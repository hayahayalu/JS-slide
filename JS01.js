...
function turnPic(){
	for(i=0;i<Num.length;i++)
		...
		distanceMove(-(index*Img[0].offsetWidth));
}
function order(){
	confine?index++:index--;
...
function doMove(distance){
	var Speed=(distance - Ul[0].offsetLeft)/10;
	Speed = Speed > 0 ? Math.ceil(Speed) : Math.floor(Speed);
	Ul[0].offsetLeft == distance ? clearInterval(timer) : Ul[0].style.left = Ul[0].offsetLeft + Speed + "px";
}
};
